...
from sicdock.motif import bb_stubs, add_xbin_to_respairdat
from sicdock.motif import add_rots_to_respairdat, get_pair_keys
from sicdock.motif import jagged_bin, logsum_bins
...
class ResPairScore:
   ...
   def __str__(self):
      return (f"ResPairScore: npdb {len(self.pdb):,} nres {len(self.aaid):,} " +
              f"npair {len(self.keys):,}\n   base Xbin: " + f"cart_resl {self.xbin.cart_resl:5.2f} "
