...
def samples_1xMonomer_orientations(resl):
    ...
    return hm.quat.quat_to_xform(quats)
...
def find_connected_1xCyclic_slide(
    spec, body, samples, min_contacts=30, contact_dis=8.0
):
...
def find_connected_2xCyclic_slide(
    spec, body1, body2, samples, min_contacts=30, contact_dis=8.0, onebody=True, **kw
):
...
def find_connected_monomer_to_cyclic_slide(
    spec, body, samples, min_contacts, contact_dis
):
    body2 = body.copy()  # shallow copy except pos
    samples_second = spec.placements_second(samples)
    maxsize = len(samples)
