...
def test_hierscore(hscore, N=1000):
    ...
    for i, hier in enumerate(hscore.hier):
        ...
        for j in range(i):
            ...
            assert olap > 0.6
            assert bfrac > 0.6
    assert np.mean(avg_ptrb_olap) > 0.8
    assert np.mean(avg_ptrb_bfrac) > 0.75
...
if __name__ == "__main__":
    hscore = sicdock.motif.hierscore.hscore_test_data(
        "10_", "/home/sheffler/debug/sicdock/hier"
    )
    test_hierscore(hscore, 10000)
