...
def append_Nhelix(userpose, alignto=7):
   ...
   first_res.extend(range(
      1, alignto + 1))  #default first 7 res of pos but user can specify if more/less
   offset = helix.size() - alignto
   protocols.toolbox.pose_manipulation.superimpose_pose_on_subset_CA(
      helix, userpose, first_res, offset)
...
def append_Chelix(userpose, alignto=7):
   ...
   core.pose.renumber_pdbinfo_based_on_conf_chains(Cterm_subpose, fix_chains=True,
                                                   start_from_existing_numbering=False)
   protocols.toolbox.pose_manipulation.superimpose_pose_on_subset_CA(
      helix, Cterm_subpose, first_res)
   core.pose.append_pose_to_pose(userpose, helix)
   return True
...
def point_in(pose, term, helixch=False):
   ...
   helixbeg = rosetta.protocols.geometry.center_of_mass(pose, pose.chain_begin(helixch),
                                                        pose.chain_begin(helixch) + 2)
   helixend = rosetta.protocols.geometry.center_of_mass(pose,
                                                        pose.chain_end(helixch) - 2,
                                                        pose.chain_end(helixch))
   if term == "N":
...
def limit_flip_update_pose(N_actual, C_actual, input_num=1, j=0, **kw):
   ...
   if (C_spec == N_spec == None) or (N_spec == N_actual and C_spec == C_actual):
      ...
      return True, None
         ...
         and ((N_actual != C_actual and N_spec == C_spec) or
   ...
   for i in range(len(kw.inputs)):
      for j in range(len(kw.inputs[i])):
         ...
         if (True in access) or (True in direction) or (False in direction):
            ...
            dir_possible, error_msg = rp.rosetta.helix_trix.limit_flip_update_pose(
               N_in, C_in, i + 1, j, **kw)
