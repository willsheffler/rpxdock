...
/**  Given a BVH, runs the query encapsulated by \a intersector.
  *  The Intersector type must provide the following members: \code
  ...
  intersects the query
     bool intersectObject(const BVH::Object &object) //returns true if the
...
/**  Given two BVH's, runs the query on their Cartesian product encapsulated by
  ...
  *  The Intersector type must provide the following members: \code
  ...
  //returns true if product of volumes intersects the query
     bool intersectVolumeObject(const BVH1::Volume &v1, const BVH2::Object &o2)
  //returns true if the volume-object product intersects the query
     bool intersectObjectVolume(const BVH1::Object &o1, const BVH2::Volume &v2)
  //returns true if the volume-object product intersects the query
     bool intersectObjectObject(const BVH1::Object &o1, const BVH2::Object &o2)
...
/**  Given a BVH, runs the query encapsulated by \a minimizer.
  ...
  *  The Minimizer type must provide the following members: \code
  ...
  necessarily the Scalar type of the BVH (if it has one)
...
/**  Given two BVH's, runs the query on their cartesian product encapsulated by
  ...
  *  The Minimizer type must provide the following members: \code
     typedef Scalar //the numeric type of what is being minimized--not
  necessarily the Scalar type of the BVH (if it has one)
     Scalar minimumOnVolumeVolume(const BVH1::Volume &v1, const BVH2::Volume
  &v2)
     Scalar minimumOnVolumeObject(const BVH1::Volume &v1, const BVH2::Object
  &o2)
     Scalar minimumOnObjectVolume(const BVH1::Object &o1, const BVH2::Volume
  &v2)
     Scalar minimumOnObjectObject(const BVH1::Object &o1, const BVH2::Object
  &o2)
  \endcode
  */
...
#ifndef EIGEN_PARSED_BY_DOXYGEN
template <typename BVH, typename Minimizer>
typename Minimizer::Scalar floormin_helper(const BVH &tree,
                                           Minimizer &minimizer,
                                           typename BVH::Index root,
                                           typename Minimizer::Scalar minimum,
                                           typename Minimizer::Scalar floor) {
  typedef typename Minimizer::Scalar Scalar;
  typedef typename BVH::Index Index;
  typedef std::pair<Scalar, Index> QueueElement;  // first element is priority
  typedef typename BVH::VolumeIterator VolIter;
  ...
  while (!todo.empty()) {
    ...
    for (; oBegin != oEnd; ++oBegin) {  // go through child objects
      minimum = (std::min)(minimum, minimizer.minimumOnObject(*oBegin));
      if (minimum <= floor) return minimum;
    }
...
template <typename Volume1, typename Object1, typename Object2,
          typename Minimizer>
struct floormin_helper1 {
  typedef typename Minimizer::Scalar Scalar;
  floormin_helper1(const Object2 &inStored, Minimizer &m)
      : stored(inStored), minimizer(m) {}
  Scalar minimumOnVolume(const Volume1 &vol) {
    return minimizer.minimumOnVolumeObject(vol, stored);
  }
 ...
 private:
  floormin_helper1 &operator=(const floormin_helper1 &);
};

template <typename Volume2, typename Object2, typename Object1,
          typename Minimizer>
struct floormin_helper2 {
  typedef typename Minimizer::Scalar Scalar;
  floormin_helper2(const Object1 &inStored, Minimizer &m)
      : stored(inStored), minimizer(m) {}
  Scalar minimumOnVolume(const Volume2 &vol) {
    return minimizer.minimumOnObjectVolume(stored, vol);
  }
 ...
 private:
  floormin_helper2 &operator=(const floormin_helper2 &);
};
...
template <typename BVH, typename Minimizer>
typename Minimizer::Scalar BVFloorMinimize(
    const BVH &tree, Minimizer &minimizer,
    typename Minimizer::Scalar floor =
        -std::numeric_limits<typename Minimizer::Scalar>::max()) {
  return internal::floormin_helper(
      tree, minimizer, tree.getRootIndex(),
      (std::numeric_limits<typename Minimizer::Scalar>::max)(), floor);
}
...
// necessarily the Scalar type of the BVH (if it has one)
// Scalar minimumOnVolumeVolume(const BVH1::Volume &v1, const BVH2::Volume
// &v2) Scalar minimumOnVolumeObject(const BVH1::Volume &v1, const BVH2::Object
// &o2)
// Scalar minimumOnObjectVolume(const BVH1::Object &o1, const BVH2::Volume
// &v2)
// Scalar minimumOnObjectObject(const BVH1::Object &o1, const BVH2::Object
...
template <typename BVH1, typename BVH2, typename Minimizer>
typename Minimizer::Scalar BVFloorMinimize(
    const BVH1 &tree1, const BVH2 &tree2, Minimizer &minimizer,
    typename Minimizer::Scalar floor =
        -std::numeric_limits<typename Minimizer::Scalar>::max()) {
  typedef typename Minimizer::Scalar Scalar;
  typedef typename BVH1::Index Index1;
  typedef typename BVH2::Index Index2;
  typedef internal::floormin_helper1<typename BVH1::Volume,
                                     typename BVH1::Object,
                                     typename BVH2::Object, Minimizer>
      Helper1;
  typedef internal::floormin_helper2<typename BVH2::Volume,
                                     typename BVH2::Object,
                                     typename BVH1::Object, Minimizer>
      Helper2;
  typedef std::pair<Scalar, std::pair<Index1, Index2> >
  ...
  while (!todo.empty()) {
    ...
    for (; oBegin1 != oEnd1;
      ...
      for (vCur2 = vBegin2; vCur2 != vEnd2;
        ...
        minimum = (std::min)(
            minimum,
            internal::floormin_helper(tree2, helper, *vCur2, minimum, floor));
        if (minimum <= floor) return minimum;
      }
    }
    ...
    for (; vBegin1 != vEnd1;
      ...
      for (oCur2 = oBegin2; oCur2 != oEnd2;
        ...
        minimum = (std::min)(
            minimum,
            internal::floormin_helper(tree1, helper, *vBegin1, minimum, floor));
        if (minimum <= floor) return minimum;
      }
