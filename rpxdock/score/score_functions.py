   ...
   #ncont_score = a * np.exp( -((ncont) - b)**2 / (2*c**2) )
      ...
      if (ub - lb) > 0:
         ncont_score = (a / (c * np.sqrt(2 * np.pi) *
                             (ub - lb))) * np.exp(-(np.log(ub - lb) - b)**2 / (2 * c**2))
...
def sasa_priority(pos1, pos2, lbub, lbub1, lbub2, ressc1, ressc2, pairs, **kw):
   ...
   prob_max = (1 / (c * np.sqrt(2 * np.pi) * (mode))) * np.exp(-(np.log(mode) - b)**2 /
                                                               (2 * c**2))
   ...
   for i, (lb, ub) in enumerate(lbub):
      if (ub - lb) > 0:
         ...
         ncont_score = (a / prob_max) * (1 / (c * np.sqrt(2 * np.pi) * (ncont_unique))) * np.exp(
            -(np.log(ncont_unique) - b)**2 / (2 * c**2))
