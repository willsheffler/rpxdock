...
def filter_sscount(xforms=None, body=None, bodiespos=None, **kw):
   ...
   if "min_element_resis" in kw.filter_sscount:
      try:
         ...
         log.warning(
            f"Could not convert min_element_resis {kw.filter['min_element_resis']} to integer, reverting to default value (1)"
         )
         min_element_resis = 1
   else:
      min_element_resis = 1
   ...
   if "confidence" in kw.filter_sscount:
      try:
         ...
         log.warning(
            f"Could not convert confidence {kw.filter_sscount['confidence']} to boolean, reverting to default value (False)"
         )
         confidence = False
   else:
      confidence = False
   ...
   if "min_ss_count" in kw.filter_sscount:
      try:
         ...
         log.warning(
            f"Could not convert min_ss_count {kw.filter_sscount['min_ss_count']} to integer, reverting to default value (3)"
         )
   else:
      min_ss_count = 3
   ...
   if "strict" in kw.filter_sscount:
      try:
         ...
         log.warning(
            f"Could not convert argument 'strict' {kw.filter_sscount['strict']} to boolean, reverting to default value (False)"
         )
   else:
      strict = False
