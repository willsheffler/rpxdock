...
def filter_sasa(xforms, body, **kw):
   ...
   if "min_sasa" in kw.filter_sasa:
      try:
         ...
         log.warning(
            f"Could not convert min sasa {kw.filter_sasa['min_sasa']} to float, reverting to default value (750)"
         )
         min_sasa = 750
   else:
      min_sasa = 750
   ...
   if "max_sasa" in kw.filter_sasa:
      try:
         ...
         log.warning(
            f"Could not convert max_sasa {kw.filter_sasa['max_sasa']} to float, reverting to default value (750)"
         )
         max_sasa = 1500
   else:
      max_sasa = 1500
   ...
   if "apply" in kw.filter_sasa:
      try:
         ...
         log.warning(
            f"Could not convert apply {kw.filter_sasa['apply']} to bool, reverting to default value (True)"
         )
         apply = True
   else:
      apply = True
   ...
   if "ncont" in kw.filter_sasa:
      try:
         ...
         log.warning(
            f"Could not convert ncont {kw.filter_sasa['ncont']} to bool, reverting to default value (False)"
         )
         ncont = False
   else:
      ncont = False
   ...
   if "max_dist" in kw.filter_sasa:
      try:
         ...
         log.warning(
            f"Could not convert max_dist {kw.filter_sasa['max_dist']} to int, reverting to default value (9)"
         )
         max_dist = 9
   else:
      max_dist = 9
   ...
   if "function" in kw.filter_sasa:
      try:
         ...
         log.warning(
            f"Could not convert function {kw.filter_sasa['function']} to string, reverting to default value (None)"
         )
         function = None
   else:
      function = None
   ...
   if "confidence" in kw.filter_sasa:
      try:
         ...
         log.warning(
            f"Could not convert confidence {kw.filter_sasa['confidence']} to boolean, reverting to default value (False)"
         )
         confidence = False
   else:
      confidence = False
   ...
   if confidence:
      return ibest[[a and b for a, b in zip(sasa_data > min_sasa, sasa_data < max_sasa)
                    ]], sasa_data
   else:
