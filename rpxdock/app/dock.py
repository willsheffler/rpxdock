...
def dock_onecomp(hscore, **kw):
    ...
    if kw.docking_method == 'grid':
        ...
        sampler = rp.sampling.grid_sym_axis(cart=np.arange(crtbnd[0], crtbnd[1],
                                                           kw.grid_resolution_cart_angstroms),
    ...
    if make_poselist:
        ...
        bodies = [
            rp.Body(pose1,
                    allowed_res=ar,
                    required_res_sets=[ar, ara],
                    modified_term=modterm,
                    og_seqlen=og_seqlen,
                    **kw) for pose1, ar, ara, modterm, og_seqlen in zip(
...
def dock_axle(hscore, **kw):
    ...
    if kw.docking_method.lower() == 'hier':
        ...
        if spec.nfold[0] == spec.nfold[1]:
            ...
            sampler1 = rp.sampling.grid_sym_axis(
                cart=np.arange(0, 100, kw.grid_resolution_cart_angstroms),
                ang=np.arange(0, 360 / (spec.nfold[0] * spec.nfold[1] / gcd(spec.nfold[0], spec.nfold[1])),
    ...
    bodies = [[rp.Body(fn, allowed_res=ar2, **kw) for fn, ar2 in zip(inp, ar)]
