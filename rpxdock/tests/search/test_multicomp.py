...
def test_cage_termini_dirs(hscore, bodyC3, bodyC2, pose_C3_1na0, pose_C2_REFS10):
   ...
   dirs = [[[C3_Ndir, C3_Cdir], [C2_Ndir, C2_Cdir]], [[not (C3_Ndir), not (C3_Cdir)], [not (C2_Ndir), not (C2_Cdir)]],
           [[not (C3_Ndir), not (C3_Cdir)], [C2_Ndir, C2_Cdir]], [[C3_Ndir, C3_Cdir], [not (C2_Ndir), not (C2_Cdir)]],
           [[None, None], [None, None]]]
   expected_flips = [[[False, False], [False, False]], [[True, True], [True, True]], [[True, True], [False, False]],
   ...
   for i, dir_pair in enumerate(dirs):
      ...
      sampler = rp.sampling.hier_multi_axis_sampler(spec, [100, 130], flip_components=kw.flip_components,
...
def test_cage_term_access(hscore, term_mod_C3_and_C2):
   ...
   for i, bodies in enumerate(body_list):
      ...
      result[i] = rp.search.make_multicomp([bodies[0], bodies[1]], spec, hscore, rp.hier_search, sampler, **kw)
   for j in range(len(result)):
      for k in range(j + 1, len(result)):
         assert not (result[j].__eq__(result[k]))
      # result[j].dump_pdbs_top_score(hscore=hscore,
