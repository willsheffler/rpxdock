...
# Edits here
def test_cage_hier_onecomp_notrim_termorient(hscore, bodyC3):
   kw = rp.app.defaults()
   ...
   spec = rp.search.DockSpec1CompCage('T3')
   # playing around to see how to change parsing of flip flag
   #if kw.flip_components == ["Cin"]:
   #   print("A")
   sampler = rp.sampling.hier_axis_sampler(spec.nfold, lb=0, ub=100, resl=10, angresl=10,
                                           axis=spec.axis, flipax=spec.flip_axis, **kw)
   result = rp.search.make_onecomp(bodyC3, spec, hscore, rp.hier_search, sampler, **kw)
...
def main():
   ...
   test_cage_hier_onecomp_notrim_termorient(hscore, C3)
