...
def test_jagged_bin():
    ...
    order, binkey, binrange = rp.motif.jagged_bin(k)
    kord = k[order]
...
def test_pair_key(respairdat):
    ...
    kij2 = xu.key_of_selected_pairs(xbin, rp.p_resi.data, rp.p_resj.data, rp.stub.data, rp.stub.data)
    kji2 = xu.key_of_selected_pairs(xbin, rp.p_resj.data, rp.p_resi.data, rp.stub.data, rp.stub.data)
    assert np.all(kij == kij2)
    assert np.all(kji == kji2)
...
def test_respairdat_addrots(respairdat):
    ...
    m, s = rp.rotamer.check_rotamer_deviation(respairdat, rotspace, quiet=1)
    assert m < 20
    assert s < 20
