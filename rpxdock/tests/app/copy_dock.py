...
def dock_onecomp(hscore, **kw):
   ...
   C_in = test_parse_flip.test_C_term_in(pdbfile, False)
   # C_in = test_parse_flip.test_C_term_in(bodies[0].pdbfile, True)
   print("C in? ", C_in)

   # N_in = test_parse_flip.test_N_term_in(bodies[0].pdbfile, False)
   N_in = test_parse_flip.test_N_term_in(pdbfile, False)
   # N_in = test_parse_flip.test_N_term_in(bodies[0].pdbfile, True)
   print("N in? ", N_in)

   kw.term = [True, True] #N_in, C_in -> take input as N_in, C_out becomes True False
   # N_in = True
   # C_in = False
   print("actual C in, N in: ", C_in, N_in, "desired C in, N in: ", kw.term[0], kw.term[1])
   ...
   if kw.docking_method == 'grid':
      ...
      ####
      if not kw.flip_components[0] or (kw.term[0] == kw.term[1] == None) or (kw.term[0] == N_in and kw.term[1] == C_in):
         ...
         print("Not possible")
         assert False
      elif (kw.term[0] != None and kw.term[0] != N_in) or (kw.term[1] != None and kw.term[1] != C_in): #N doesnt match
         #flip once
