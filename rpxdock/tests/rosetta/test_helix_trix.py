...
def test_append_helix(pose, helix):
...
def test_termini_direction(pose_list):
...
def test_remove_helix(pose, helix):
   og_pose = rosetta.core.pose.Pose().assign(pose)
   test_append_helix(pose, helix)
...
def test_limit_flip_update():
   ...
   kw.termini_dir = [[[None, None]], [[True, False]], 
...
def test_init_termini(pose_list, helix):
   kw = rp.app.defaults()
   ...
   kw.termini_dir = [[[None, None]], [[True, True]], [[False, False]],
                     [[True, False]], [[None, None]], [[True, False]]]
   kw.term_access = [[[False, False]], [[False,False]], [[True, False]],
   ...
   assert og_lens == [[None],[poses[1][0].size()],
                     [poses[2][0].size() - helix.size()],
                     [poses[3][0].size() - helix.size()],
                     [poses[4][0].size() - (helix.size() * 2)],
                     [poses[4][0].size() - (helix.size() * 2)]]
   assert (type(poses[1][0]) == type(poses[2][0]) == type(poses[3][0]) 
