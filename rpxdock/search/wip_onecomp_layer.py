   ...
   **kw,
   ...
   for k, v in extra.items():
      if not isinstance(v, (list, tuple)) or len(v) > 3:
         v = ['model'], v
      data[k] = v
   # data['disp'] = (['model'], np.sum(xforms[:, :3, 3] * spec.cartaxis[None, :3],
   # axis=-1).squeeze())
...
class OneCompEvaluator:
   ...
   def __init__(
         ...
         spec,
         hscore,
         wts=Bunch(ncontact=0.1, rpx=1.0),
         trimmable_components="AB",
         **kw,
   ):
   ...
   def __call__(self, xforms, iresl=-1, wts={}, **kw):
      ...
      to_neighbor_olig = hm.hrot([0, 0, 1], np.pi)
      Xsym = to_neighbor_olig @ X
      Xsym[..., :, 3] -= X[..., :, 3]
      X[..., :, 3] -= X[..., :, 3]
