...
class ZeroDHier:
    ...
    def get_xforms(self, resl=0, idx=None, full=False):
...
class CompoundHier:
    ...
    def get_xforms(self, resl=0, idx=None, full=False):
        ...
        for p, sidx in zip(self.parts, split):
            v, x = p.get_xforms(resl, sidx, full=True)
            # ic(p, len(sidx), x.shape)
            assert len(x) == len(sidx)
            ok &= v
            xforms.append(x)
        if full: return ok, np.stack(xforms, axis=1)
        else: return ok, np.stack(xforms, axis=1)[ok]
