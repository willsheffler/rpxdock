...
class LineHier:
    ...
    def get_xforms(self, resl, idx, full=False):
        ok, disp = self.hier1d.get_trans(resl, idx, full=full)
...
def hier_axis_sampler(
   ...
   '''
    ...
    if fixed_rot == [0]:
        ...
        samp = rp.sampling.RotCart1Hier_f4(fw_cartlb, fw_cartub, cart_nstep, fw_rotlb, fw_rotub, ang_nstep,
                                           axis[:3])
    else:
    ...
    **kw,
    ...
    for i in range(len(spec.nfold)):
        if spec.comp_is_dihedral[i]:
            ...
            s = rp.sampling.RotCart1Hier_f4(cart_bounds[i, 0], cart_bounds[i, 1], cart_nstep[i], 0, ang[i],
                                            ang_nstep[i], spec.axis[i][:3])
        samp.append(s)
    ...
    sampler.attrs = dict(spec=spec,
                         cart_bounds=cart_bounds,
                         resl=resl,
                         angresl=angresl,
                         flip_components=flip_components)
    return sampler
...
def hier_mirror_lattice_sampler(
   ...
   '''
    ...
    sampaxis = rp.sampling.RotCart1Hier_f4(cart_bounds[1, 0], cart_bounds[1, 1], cart_nstep[1], 0, ang[0],
                                           ang_nstep[0], [0, 0, 1])
